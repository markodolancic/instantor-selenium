...
public interface RemoteProxy extends Comparable<RemoteProxy> {
    ...
    /**
     * Return how many times proxy was opening new session.
     *
     * @return count of done jobs (sessions)
     */
    int getRunCount();

    /**
     * Return maximum number of allowed jobs. After maximum is reached, proxy will be disabled and restarted.
     * <p/>
     * If less than 0, proxy will never be disabled/restarted.
     *
     * @return count of maximum jobs (sessions)
     */
    int getRunCountMax();

    /**
     * If proxy can be used to start sessions. If false, proxy should return null as result of call to getNewSession method.
     *
     * @return proxy is enabled or not
     */
    boolean isEnabled();

    /**
     * Change enabled state of proxy
     *
